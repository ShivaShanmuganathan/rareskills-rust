...
pub fn read_owner(env: &Env, id: i128) -> Address {
    ...
    match env.storage().persistent().get::<DataKey, Address>(&key) {
        // @audit-issue - Why are we using balance here?
        Some(balance) => {
...
// @audit-issue - There is no access control on write_owner
...
// @audit-issue - Isn't it efficient to store the owned ids in vector and return the vector, rather than iterating over the supply, checking owner and pushing to vector?
